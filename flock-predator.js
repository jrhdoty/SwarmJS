...
var boundaryAvoidance = {
  ...
  strength: 5,
  calculate: function(agent, neighbors){
    bounds =  [20, 20, window.innerHeight-20, window.innerWidth-20];
    var result = new Vector(0, 0);
...
for(var i = 0; i < Math.floor(Math.random()*10); i++){
  optsRaptor.position = new Vector(Math.random()*window.innerWidth, Math.random()*window.innerHeight);
  optsRaptor.velocity = new Vector((Math.random()-.5)*1, (Math.random()-.5)*1);
  population.push(new Agent(optsRaptor));
}
